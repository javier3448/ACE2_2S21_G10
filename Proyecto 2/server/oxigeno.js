...
function getReport(tempo, repeticion, dateTime) {
  ...
  let aux = tempo;
  aux.sort(dynamicSort("oxigeno"));
  let maxData = aux[aux.length-1].oxigeno;
  let minData = aux[0].oxigeno
  let sum = 0;
  for (let index = 0; index < tempo.length; index++) {
    sum += tempo[index].oxigeno;
  }
  sum = sum/tempo.length;
  sum = sum.toFixed(2);
  let tiempo = end(dateTime);
  let result = {arrayOxigeno: aux, avgOxigeno: sum, maxOxigeno: maxData, minOxigeno: minData, repeticion: repeticion, tiempoEnMinutos: tiempo };
  ...
  while (true) {
    ...
    if (index + 1 > dataDistancia.length - 1) {
      if (dataDistancia.length === 1) {
        temporal.push({oxigeno: dataDistancia[index].oxigeno});
