...
function getReport(tempo, repeticion, dateTime) {
  ...
  let aux = tempo;
  aux.sort(dynamicSort("temperatura"));
  let maxData = aux[aux.length-1].temperatura;
  let minData = aux[0].temperatura
  let sum = 0;
  for (let index = 0; index < tempo.length; index++) {
    sum += tempo[index].temperatura;
  }
  sum = sum/tempo.length;
  sum = sum.toFixed(2);
  let tiempo = end(dateTime);
  let result = {arrayTemperatura: aux, avgTemperatura: sum, maxTemperatura: maxData, minTemperatura: minData, repeticion: repeticion, tiempoEnMinutos: tiempo };
  ...
  while (true) {
    ...
    if (index + 1 > dataDistancia.length - 1) {
      if (dataDistancia.length === 1) {
        temporal.push({temperatura: dataDistancia[index].temperatura});
